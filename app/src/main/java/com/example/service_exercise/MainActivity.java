package com.example.service_exercise;
...
public class MainActivity extends AppCompatActivity {
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            if (myBinder.isPlaying()) {
                btPlay.setBackgroundResource(R.drawable.ic_baseline_library_music_24);
            } else {
                btPlay.setBackgroundResource(R.drawable.ic_baseline_play_circle_outline_24);
            }
    ...
    private void setIsPlayButton(Button bt) {
        if (myBinder.isPlaying()) {
            myBinder.pauseMusic();
            bt.setBackgroundResource(R.drawable.ic_baseline_play_circle_outline_24);
        } else {
            myBinder.playMusic();
            bt.setBackgroundResource(R.drawable.ic_baseline_stop_24);
        }
    }
    /**畫面消失時，則不監聽目前播放狀態(解除綁定)*/
    @Override
